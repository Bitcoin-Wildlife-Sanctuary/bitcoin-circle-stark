...
pub fn generate_cs(
    ...
    for i in 0..4 {
        column_line_interaction_shifted_vars.push((
            worm.read(format!("column_line_coeffs_interaction_shifted_{}_a", i))?,
            worm.read(format!("column_line_coeffs_interaction_shifted_{}_b", i))?,
        ))
    }
    ...
    let denominator_inverse_shifted_l: CM31Var =
        worm.read(format!("denominator_inverse_shifted_{}_l", query_idx))?;
    let denominator_inverse_shifted_r: CM31Var =
        worm.read(format!("denominator_inverse_shifted_{}_r", query_idx))?;
    ...
    worm.write(format!("quotient_term2_{}_l", query_idx), &quotient_term2_l)?;
    worm.write(format!("quotient_term2_{}_r", query_idx), &quotient_term2_r)?;
