...
pub fn generate_cs(
    ...
    trace_queried_results.push((
        worm.read(format!("trace_mult_queried_results_{}_l", query_idx))?,
        worm.read(format!("trace_mult_queried_results_{}_r", query_idx))?,
    ));
    trace_queried_results.push((
        worm.read(format!("trace_a_val_queried_results_{}_l", query_idx))?,
        worm.read(format!("trace_a_val_queried_results_{}_r", query_idx))?,
    ));
    trace_queried_results.push((
        worm.read(format!("trace_b_val_queried_results_{}_l", query_idx))?,
        worm.read(format!("trace_b_val_queried_results_{}_r", query_idx))?,
    ));
    trace_queried_results.push((
        worm.read(format!("trace_c_val_queried_results_{}_l", query_idx))?,
        worm.read(format!("trace_c_val_queried_results_{}_r", query_idx))?,
    ));

    let mut column_line_trace_vars = Vec::<(CM31Var, CM31Var)>::new();
    for i in 0..4 {
        column_line_trace_vars.push((
            worm.read(format!("column_line_coeffs_trace_{}_a", i))?,
            worm.read(format!("column_line_coeffs_trace_{}_b", i))?,
        ))
    }
    ...
    let alpha20: QM31Var = worm.read("line_batch_random_coeff_20")?;

    let alpha20trace_l = &alpha20 * (&table, &sum_num_trace_l);
    let alpha20trace_r = &alpha20 * (&table, &sum_num_trace_r);

    worm.write(format!("alpha20trace_{}_l", query_idx), &alpha20trace_l)?;
    worm.write(format!("alpha20trace_{}_r", query_idx), &alpha20trace_r)?;

    let prepared_oods_a: CM31Var = worm.read("prepared_oods_a")?;
    let prepared_oods_b: CM31Var = worm.read("prepared_oods_b")?;
