...
pub fn generate_cs(
    ...
    let quotient_term1_num_l: QM31Var = worm.read(format!("quotient_term1_num_{}_l", query_idx))?;
    let quotient_term1_num_r: QM31Var = worm.read(format!("quotient_term1_num_{}_r", query_idx))?;

    let denominator_inverse_l: CM31Var =
        worm.read(format!("denominator_inverse_{}_l", query_idx))?;
    let denominator_inverse_r: CM31Var =
        worm.read(format!("denominator_inverse_{}_r", query_idx))?;
    ...
    let quotient_term2_l: QM31Var = worm.read(format!("quotient_term2_{}_l", query_idx))?;
    let quotient_term2_r: QM31Var = worm.read(format!("quotient_term2_{}_r", query_idx))?;
    ...
    let fri_fold_random_coeff_var: QM31Var = worm.read("fri_fold_random_coeff")?;
    ...
    let expected_entry_quotient: QM31Var =
        worm.read(format!("expected_entry_quotient_{}", query_idx))?;
